...
from JaNa_Robot import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("./JaNa_Robot/resources/fonts/*")
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [JaNa Robot](http://t.me/JaNa_RoBoT)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./JaNa_Robot/resources/fonts"):
            os.remove(font_)
