...
import sys
import traceback
import JaNa_Robot.modules.sql.users_sql as sql
...
from JaNa_Robot.modules import ALL_MODULES
from JaNa_Robot.modules.helper_funcs.chat_status import is_user_admin
from JaNa_Robot.modules.helper_funcs.misc import paginate_modules
...
PM_START_TEXT = """
*Hello {} !*
✪ I'm an Anime-Theme Management Bot [✨](https://telegra.ph/file/66f4cbd1978ddd57cd034.jpg)
────────────────────────
× *Uptime:* `{}`
× `{}` *users, across* `{}` *chats.*
────────────────────────
...
DONATE_STRING = """Heya, glad to hear you want to donate!
 You can support the project by contacting @Shiikariii \
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("JaNa_Robot.modules." + module_name)
...
def jana_about_callback(update, context):
    query = update.callback_query
    if query.data == "jana_":
        query.message.edit_text(
    ...
    elif query.data == "jana_support":
        query.message.edit_text(
            text="*๏ jana support chats*"
...
def main():
    ...
    start_callback_handler = CallbackQueryHandler(
        start_back, pattern=r"jana_back", run_async=True
    )
    ...
    about_callback_handler = CallbackQueryHandler(
        jana_about_callback, pattern=r"jana_", run_async=True
    )
